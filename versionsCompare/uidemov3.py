...
class PDFComparer(QMainWindow):
    ...
    def highlight_differences(self, doc, words1, words2, page_num, is_primary):
        differences = []
        if page_num < len(words1):
            ...
            for word1 in words1[page_num]:
                if word1[4] not in words2_set:  # Diferencia detectada
            ...
            if not is_primary:
                for word2 in words2[page_num]:
                    if word2[4] not in words1_set:  # Diferencia detectada en PDF2
    ...
    def load_page_pair(self, page_num):
        ...
        doc1, differences1 = self.highlight_differences(doc1, self.pdf1_words, self.pdf2_words, page_num, True)

        # Cargar y resaltar diferencias en PDF2
        doc2 = fitz.open(self.pdf2_path)
        doc2, differences2 = self.highlight_differences(doc2, self.pdf2_words, self.pdf1_words, page_num, False)
    ...
    def highlight_current_difference(self):
        if self.current_difference_index >= 0 and self.current_difference_index < len(self.differences):
            ...
            text_pdf1 = word1[4] if word1 else 'SD'
            text_pdf2 = word2[4] if word2 else 'SD'
            self.difference_label.setText(f"PDF1: '{text_pdf1}'\nPDF2: '{text_pdf2}'")
