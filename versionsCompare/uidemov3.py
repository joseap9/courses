...
class PDFComparer(QMainWindow):
    ...
    def highlight_differences(self, doc, words1, words2, page_num):
        ...
        if page_num < len(words1) and page_num < len(words2):
            ...
            # Definir el umbral de exclusión (10% desde el final de la página)
            exclusion_threshold = page_height * 0.9
            ...
            for word1 in words1[page_num]:
                ...
                if word_rect.y1 > exclusion_threshold:
                    continue  # Saltar las palabras que están en las dos últimas líneas (parte inferior de la página)
    ...
    def download_excel(self):
        # Solicitar el nombre del responsable antes de descargar
        ...
        options = QFileDialog.Options()
        fileName, _ = QFileDialog.getSaveFileName(self, "Guardar Archivo Excel", "", "Excel Files (*.xlsx);;All Files (*)", options=options)
        if fileName:
